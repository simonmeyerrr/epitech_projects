...
MODULE_EXPORTER(FileServer)

const std::tuple<std::string, std::string> type[] = {
    ...
    {".ts", "application/typescript"}, {".ttf", "font/ttf"},
    {".vsd", "application/vnd.visio"}, {".wav", "audio/x-wav"},
...
void FileServer::setLogger(api::module::ALogger *logger)
{
    updateLogger(logger);
}

void FileServer::setConfig(const api::JsonObject &config)
{
    if (config == _config)
        return;
    _config = config;
    std::wstring url = L".";
    try {
        url = _config.at(L"url").as<api::JsonString>();
    } catch (...) {}
    _url = std::string(url.begin(), url.end());
}

bool FileServer::handle(Connection &connection)
{
    if (connection.res->getStatus() != 200)
        return true;
    api::String path = _url + connection.req->getUri();
    api::Buffer content;
    ...
    ifs.close();
    connection.res->setBody(content);
    std::string extension = path.substr(path.rfind('.'));
    for (int i = 0; !content.empty() && i < 6; ++i)
        if (extension == std::get<0>(type[i]))
            connection.res->getHeaders()["Content-Type"] = std::get<1>(type[i]);
    connection.res->setStatus(content.empty() ? 404 : 200);
    log("Serving this file: " + path, api::module::ALogger::LEVEL_DEBUG);
    return true;
}