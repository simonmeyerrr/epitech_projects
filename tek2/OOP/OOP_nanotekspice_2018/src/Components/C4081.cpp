...
{
    std::cout << "4081 " << getName() << ":" << std::endl;
    for (size_t i = 1; i <= 14; i++) {
        std::cout << "-pin" << i << ": ";
        if (getPinType(i) == OUT)
            std::cout << "output";
...
void nts::C4081::updateState(nts::Tristate state)
{
    (void)state;
}

nts::PinType nts::C4081::getPinType(std::size_t pin) const
{
...
nts::Tristate nts::C4081::exec_compute(std::size_t pin1, std::size_t pin2)
{
    ...
    return (Gates::andGate(link1->component.compute(link1->pin),
