...
{
    std::cout << "4001 " << getName() << ":" << std::endl;
    for (size_t i = 1; i <= 14; i++) {
        std::cout << "-pin" << i << ": ";
        if (getPinType(i) == OUT)
            std::cout << "output";
...
void nts::C4001::updateState(nts::Tristate state)
{
    (void)state;
}

nts::PinType nts::C4001::getPinType(std::size_t pin) const
{
    const PinType pins[] = {NONE, IN, IN, OUT, OUT, IN, IN, NONE, IN, IN, OUT,
        OUT, IN, IN, NONE};
    if (pin <= 14)
        return (pins[pin]);
    return (NONE);
...
nts::Tristate nts::C4001::exec_compute(std::size_t pin1, std::size_t pin2)
{
