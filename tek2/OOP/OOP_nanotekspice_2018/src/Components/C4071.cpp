...
void nts::C4071::setLink(std::size_t pin, nts::IComponent &other, std::size_t other_pin)
{
...
{
    std::cout << "4071 " << getName() << std::endl;
    for (size_t i = 1; i <= 14; i++) {
        std::cout << "-pin" << i << ": ";
        if (getPinType(i) == OUT)
            std::cout << "output";
...
void nts::C4071::updateState(nts::Tristate state)
{
    (void)state;
}

nts::PinType nts::C4071::getPinType(std::size_t pin) const
{
...
nts::Tristate nts::C4071::exec_compute(std::size_t pin1, std::size_t pin2)
{
    ...
    nts::Tristate value = Gates::orGate(link1->component.compute(link1->pin),
