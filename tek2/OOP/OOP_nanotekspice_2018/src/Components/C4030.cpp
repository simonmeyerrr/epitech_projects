...
void nts::C4030::setLink(std::size_t pin, nts::IComponent &other, std::size_t other_pin)
{
...
{
    std::cout << "4030 " << getName() << ":" << std::endl;
    for (size_t i = 1; i <= 14; i++) {
        std::cout << "-pin" << i << ": ";
        if (getPinType(i) == OUT)
            std::cout << "output";
...
void nts::C4030::updateState(nts::Tristate state)
{
    (void)state;
}

nts::PinType nts::C4030::getPinType(std::size_t pin) const
{
...
nts::Tristate nts::C4030::exec_compute(std::size_t pin1, std::size_t pin2)
{
    ...
    nts::Tristate value = Gates::xorGate(link1->component.compute(link1->pin),
