...
Test(list_add_elem_at_back, begin_null)
{
    list_t begin = NULL;

    cr_assert_eq(TRUE, list_add_elem_at_back(&begin, 1));
    cr_assert_not_null(begin);
    cr_assert_null(begin->next);
    cr_assert_eq(begin->value, 1);
}

Test(list_add_elem_at_back, one)
{
    node_t elem1 = {3, NULL};
    list_t begin = &elem1;
    ...
    cr_assert_eq(begin->value, 3);
    cr_assert_eq(begin->next->value, 1);
}

Test(list_add_elem_at_back, normal)
{
    node_t elem4 = {3, NULL};
    ...
    cr_assert_eq(begin->next->next->next->next->value, 1);
}