...
Test(list_del_elem_at_back, begin_null)
{
    list_t begin = NULL;

    cr_assert_eq(FALSE, list_del_elem_at_back(&begin));
    cr_assert_null(begin);
}

Test(list_del_elem_at_back, one)
{
    node_t elem1 = {3, NULL};
    list_t begin = &elem1;

    cr_assert_eq(TRUE, list_del_elem_at_back(&begin));
    cr_assert_null(begin);
}

Test(list_del_elem_at_back, normal)
{
    node_t elem4 = {3, NULL};
