...
void touch_up(int touch, char** map, var_t* var)
{
	if (var->player_y < 2)
		return;
	var->next1 = (my_strlen(map[var->player_y - 1]) >= var->player_x - 1) ?
	(map[var->player_y - 1][var->player_x]) : ('\0');
	var->next2 = (my_strlen(map[var->player_y - 2]) >= var->player_x - 1) ?
	(map[var->player_y - 2][var->player_x]) : ('\0');
...
{
	if (var->player_x > my_strlen(map[var->player_y])  - 2)
		return;
