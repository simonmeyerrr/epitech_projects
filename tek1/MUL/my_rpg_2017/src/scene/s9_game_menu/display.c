...
static void display_game_menu2(global_t *global, my_sprite_t *sprite,
my_font_t *font, sfRenderWindow *win)
{
	sfVector2f pos = get_mouse_pos(global);

	my_draw_text(win, TEXT(VEC(650, 270), 50, sfBlack, font->font2),
	"Reprendre", 0);
	my_draw_text(win, TEXT(VEC(670, 420), 50, sfBlack, font->font2),
	"   Aide   ", 0);
	my_draw_text(win, TEXT(VEC(670, 570), 50, sfBlack, font->font2),
	"Reglages", 0);
	my_draw_text(win, TEXT(VEC(565, 720), 50, sfBlack, font->font2),
	"Se deconnecter", 0);
	for (int i = 0 ; i < 4 ; i++) {
...
void display_game_menu(global_t *global, my_sprite_t *sprite, my_font_t *font,
sfRenderWindow *win)
{
	...
	display_game_menu2(global, sprite, font, win);
}
